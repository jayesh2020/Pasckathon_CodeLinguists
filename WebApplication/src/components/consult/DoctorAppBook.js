...
import { bookAppointment,getDoctor } from '../../actions/doctorSearch';
...
*/
    ...
    const bookApp = (e) => {
        ...
        if(timeSlot && selectDate) {
            bookAppointment({timeSlot, dateOf: selectDate, report:currentReport,selectedDoctor});
            clearCurrentReport();
   ...
   const [slots,setSlots] = useState([]);
    return (
        <div>
            {currentReport && selectedDoctor && <Fragment>
            <Card style={{ marginLeft: "auto", marginTop:"20px",marginRight:"auto",width: '30rem',borderRadius:"5px" }}>
                <Card.Img variant="top" src={currentReport.doctorUrl} thumbnail style={{maxHeight:"300px",paddingRight:"10px", paddingLeft:"10px", marginTop:"20px", height:"300px", width:"100%", display:"block"}} />
                <Card.Body>
              ...
              {toggler && <Button variant="orange text-white bold" onClick={bookApp}>Submit</Button>}
                </Card.Body>
            </Card>
            </Fragment>}
        </div>
...
export default connect(mapStateToProps,mapDispatchToProps)(DoctorAppBook);